...
const TYPE_ICONS: Record<SearchItem["type"], React.ReactNode> = {
  ...
  deal: (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      ...
      <path d="M7 4.5v3l2 1.5" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  ),
  page: (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
...
function SearchModal({ onClose }: { onClose: () => void }) {
  ...
  const results = query.trim().length >= 2
    ...
    : SEARCH_INDEX.filter((i) => i.type === "tool").concat(
        SEARCH_INDEX.filter((i) => i.type === "guide").slice(0, 2)
      );
  ...
  return (
    <>
      ...
      {/* Modal */}
      <div
        className="fixed left-1/2 z-[101] w-full max-w-xl -translate-x-1/2 rounded-3xl overflow-hidden border shadow-2xl"
        style={{
          top: "12vh",
          background: "var(--card)",
          borderColor: "var(--border)",
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b" style={{ borderColor: "var(--border)" }}>
          ...
          />
          {query && (
            <button onClick={() => setQuery("")} className="text-xs px-2 py-1 rounded-lg" style={{ color: "var(--ink-40)", background: "var(--muted)" }}>
              Clear
            </button>
          )}
        </div>
        ...
        <div className="max-h-[60vh] overflow-y-auto">
          {results.length === 0 ? (
            <div className="px-5 py-10 text-center">
              <p className="text-sm" style={{ color: "var(--ink-40)" }}>No results for &ldquo;{query}&rdquo;</p>
            </div>
          ) : (
            <>
              {!query.trim() && (
                ...
                {results.map((item, i) => (
                  <li key={item.href}>
                    <button
                      ...
                      style={{
                        background: i === activeIdx ? "var(--muted)" : "transparent",
                      }}
                    >
        ...
        <div className="flex items-center gap-4 px-5 py-3 border-t" style={{ borderColor: "var(--border)", background: "var(--muted)" }}>
          {[["↵", "select"], ["↑↓", "navigate"], ["esc", "close"]].map(([key, label]) => (
            <span key={label} className="flex items-center gap-1.5 text-xs" style={{ color: "var(--ink-40)" }}>
              <kbd className="px-1.5 py-0.5 rounded text-[10px] font-semibold border" style={{ borderColor: "var(--border)", background: "var(--card)" }}>{key}</kbd>
              {label}
            </span>
          ))}
        </div>
